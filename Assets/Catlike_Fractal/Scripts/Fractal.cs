...
public class Fractal : MonoBehaviour
{
    ...
    float maxSagAngleA, maxSagAngleB;

    [SerializeField, Range(0, 90f)]
    float spinSpeedA, spinSpeedB;

    [SerializeField, Range(0, 1f)]
    float reverseSpinChange = 0.25f;
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < matricesBuffers.Length; i++)
        {
            ...
            {
                float gradientProgress = i / (matricesBuffers.Length - 1f);
                colorA = gradientA.Evaluate(gradientProgress);
                colorB = gradientB.Evaluate(gradientProgress);
                instanceMesh = mesh;
            }
    ...
    private FractalPart CreatePart(int childIndex) => new FractalPart
    {
        ...
        spinVelocity = (Random.value < reverseSpinChange ? -1f : 1f) *
